...
export default function Navbar({title="LC Predictor",cb}){
    const navigate = useNavigate();
    return (
        <Card style={{display:"flex","margin":"20px 20px",justifyContent:"space-between"}} className={"flex justify-between mx-10 mt-5"}>
            <Button className={"justify-start"} color={"gray"} onClick={cb === undefined ? ()=>navigate('/'):cb}>
            ...
            <h4 className={"text-2xl"} style={{marginRight:"40px"}}>{title}</h4>
            <span></span>
        </Card>
    )
}